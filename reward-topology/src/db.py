...
# Reward data connection parameters
DB_NAME = os.getenv("POSTGRES_DB", "reward_topology_db")
...
def initialize_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS reward_topology_data (
...
def save_reward_result(iteration, total_reward, breakdown):
    ...
    json_file_path = f"/app/src/assets/reward_topology_results_{iteration}.json"
    ...
    cur.execute("""
        INSERT INTO reward_topology_data (iteration, total_reward, json_path)
        VALUES (%s, %s, %s)
...
def get_latest_reward_iteration():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT iteration FROM reward_topology_data ORDER BY iteration DESC LIMIT 1;")
    row = cur.fetchone()
    cur.close()
    conn.close()
